...
export async function evolutionWebhookTool(req: Request, res: Response) {
  ...
  try {
    ...
    if (!expectedSecret) {
      console.warn("EVOLUTION_WEBHOOK_SECRET not configured");
      return res.status(500).json({ ok: false });
    }

    if (!event || event.secret !== expectedSecret) {
      console.warn(" Invalid Evolution webhook secret");
    ...
    // --------------------------------------------------
    if (!event.event) {
      console.warn(" Invalid Evolution payload (missing event)");
    ...
    if (n8nWebhookUrl) {
      ...
      console.warn(" N8N_WEBHOOK_URL not configured, skipping forward to n8n");
    }
